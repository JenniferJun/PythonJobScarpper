...
def parsing_web3(soup_content):
    jobs_list = soup_content.find("tbody", class_="tbody").find_all("tr", class_="table_row")

    jobs = []
    skills = [] 
    for job in jobs_list:
        try:
            title = job.find("div", class_="job-title-mobile").find("h2").text.strip()
            title_url= job.find("div", class_="job-title-mobile").find("a")['href']
            company = job.find("td", class_="job-location-mobile").find("h3").text.strip()
            company_url= job.find("td", class_="job-location-mobile").find("a")['href']
            
            description = ""
            if job.find("td", class_="job-location-mobile"):
                description_a = job.findAll("td", class_="job-location-mobile") 
                description = ", ".join([d.text.strip() for d in description_a])
            
            skill_area = job.find_all('span', class_='my-badge my-badge-secondary')
        ...
        for jobs_list in jobs_lists:
            ...
            for job in jobs_list:
                try:    
                    ...
                    if(job.find("div", class_="new-listing__header").find("h4", class_="new-listing__header__title")):
                        title = job.find("div", class_="new-listing__header").find("h4", class_="new-listing__header__title").text.strip()
                    else:
                        continue
                    
                    title_url = job.find("a")['href']
                    company = job.find("p", class_="new-listing__company-name").text.strip()
                    company_url = job.find("div", class_="tooltip--flag-logo").find("a")['href'] if job.find("div", class_="tooltip--flag-logo") else ""
        
                    description = ""
                    if job.find("div", class_="new-listing__categories").find("p"):
                        description_a = job.find("div", class_="new-listing__categories").findAll("p")
                        description = ", ".join([d.text.strip() for d in description_a])
